 ...
 */
function applyRotationToCollisionBox(relBox, metaData, targetBox) {
    // 1. メタデータの解析
    const rotation = metaData & 3;      // 下位2ビット：0-3の回転
    const isUpsideDown = (metaData & 4) !== 0; // 3ビット目：上下反転フラグ

    targetBox.copy(relBox);

    // --- ステップ1: 上下反転の適用 ---
    if (isUpsideDown) {
        // ブロックの中心 0.5 を軸に Y座標を反転させる
        // y' = 0.5 - (y - 0.5) => y' = 1.0 - y
